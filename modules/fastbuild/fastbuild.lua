...
--
	...
	function fastbuild.writeCompiler(cfg, name)
		...
		local linker, compiler
		if toolset == p.tools.msc then
			compiler = cxx
			linker = toolset.gettoolname(cfg, "link", architecture)
		else
			local function absPath(tool)
				if path.isabsolute(tool) then
					return tool
				end
				local found = os.outputof("which " .. tool, "output")
				if found and #found > 0 then
					return found
				end
				return tool
			end
			-- Fastbuild requires specifying the full path to compiler executables not just the name
			cc = absPath(cc)
			cxx = absPath(cxx)
			ar = absPath(ar)
			compiler = iif(cfg.language == p.C, cc, cxx)
			linker = compiler
		end
	...
	function fastbuild.writeObjectList(prj, cfg, files, name)
		...
		p.w(".Compiler = %s", iif(cfg.language == "C", ".CC", ".CCX"))
	...
	function fastbuild.writeCompileOptions(prj, cfg, filecfg)
		...
		local ccOpts
		if toolset == p.tools.msc then
			ccOpts = iif(cfg.language == "C", "/Tc", "/Tp")..'"%1" /Fo"%2" /nologo /c'
		else
			ccOpts =  '-c "%1" -o "%2"'
		end
		p.w(".CompilerOptions = '%s %s'", ccOpts, fastbuild.getcxxflags(toolset, cfg, filecfg))
